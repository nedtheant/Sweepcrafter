...
setblock ~ 109 ~ stone
setblock ~ 110 ~ repeating_command_block{Command:"execute if entity @e[type=armor_stand,distance=..1] run spreadplayers 24.5 24.5 1 24.5 false @e[type=armor_stand]",auto:1}


# Set Amount of mines to spawn
scoreboard players set variables mines 515
...
setblock ~ ~-3 ~ chain_command_block[facing=down,conditional=true]{Command:"execute if score variables mines matches 0 run spreadplayers 24.5 24.5 1 24.5 false @e[type=armor_stand]",auto:1}
...
setblock ~2 ~-5 ~ command_block[facing=east]{Command:"function mine_gen:50x50_515/third_stage"}
