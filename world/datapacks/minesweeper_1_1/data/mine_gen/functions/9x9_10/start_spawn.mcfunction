...
setblock ~ 109 ~ stone
setblock ~ 110 ~ repeating_command_block{Command:"execute if entity @e[type=armor_stand,distance=..1] run spreadplayers 4 4 1 4 false @e[type=armor_stand]",auto:1}


# Set Amount of mines to spawn
scoreboard players set variables mines 10
...
setblock ~ ~-3 ~ chain_command_block[facing=down,conditional=true]{Command:"execute if score variables mines matches 0 run spreadplayers 4 4 1 4 false @e[type=armor_stand]",auto:1}
...
setblock ~2 ~-5 ~ command_block[facing=east]{Command:"function mine_gen:9x9_10/third_stage"}
