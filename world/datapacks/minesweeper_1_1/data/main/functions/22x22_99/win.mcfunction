...
# Setup win_state
fill 0 105 0 21 105 21 repeating_command_block{Command:"function main:win_state",auto:1}
scoreboard players set variables flagged 0

#highscore thingo
execute if score variables timer < variables exp_bt run say New Best Time!
execute if score variables timer < variables exp_bt run scoreboard players operation variables exp_bt = variables timer