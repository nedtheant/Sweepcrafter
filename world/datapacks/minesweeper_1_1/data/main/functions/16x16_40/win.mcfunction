...
# Setup win_state
fill 0 105 0 15 105 15 repeating_command_block{Command:"function main:win_state",auto:1}
scoreboard players set variables flagged 0

#highscore thingo
execute if score variables timer < variables int_bt run say New Best Time!
execute if score variables timer < variables int_bt run scoreboard players operation variables int_bt = variables timer